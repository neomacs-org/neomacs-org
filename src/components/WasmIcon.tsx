/* lucide has no WebAssembly glyph, so this is the official mark from
   simple-icons, drawn in currentColor so callers colour it via `color`.
   Shared rather than duplicated: it appears in the hero button and again in
   the mockup window's title bar. */
const WasmIcon = ({ size = 18 }: { size?: number }) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
